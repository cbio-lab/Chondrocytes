...
library(DOSE)
library(DT)
#setwd("/Users/username/Desktop/GitHub/Chondro")
```
...
#| warning: false
#| cache: true
meta_data_chondro <- read.xlsx("chondro_metadata/chondro_metadata_1.xlsx", sheet= T, sep="\t")
...
В данной сформированы метаданные для проведения исследования. При формировании метаданных учитывались следующие парамтеры: "run" (парное или единичное прочтение), "cell" (клетки), "type" (2d или 3d культуры), "paired" (прочтения), "group" (состояние нормы и патологии).
...
По результатам дифференциальной экспрессии генов мы можем оценить их распределение и значимость на графике, а также величину экспрессии в различных состояниях по табличным данным. Образцы на графике PCA соотвествуют двум контрольным группам: case (патология) и control (норма), являются хондроцитами (от пациента, 2d-культура).Данные группы образцов относятся к коксартрозу и на основании графических данных можно сделать вывод, что вклад патологии (тазобедренный сустав) является существенным и может указывать на существенные биологические различия на транскриптомном уровне. Образцы:

-   E7_S55_L001_R1_001 69.102510 16.273896 control;

-   E7_S55_L002_R1_001 67.235903 14.323059 control;

-   E3_S51_L001_R1_001 64.084555 -18.383523 case;

-   E3_S51_L002_R1_001 62.575896 -19.545576 case.
...
![](case_vs_control/UR/(+)Mixed, incl. Amplification of signal from the kinetochores, and Condensin complex.png)

(<https://string-db.org/cgi/network?taskId=bKqFpAJk8Pc6&sessionId=brBtz9pZBebx>)

![](case_vs_control/UR/(+)(cl1)Mixed, incl. Amplification of signal from the kinetochores, and Condensin complex.svg)

(<https://string-db.org/cgi/network?taskId=bQO5i04z3pAD&sessionId=brBtz9pZBebx>)
...
datatable(as.data.frame(custome_UR_1),
          options = list(
            scrollX = TRUE,      
            scrollY = "400px",   
            pageLength = 10
...
![](case_vs_control/DR/(-)Extracellular matrix organization+Mixed, incl. Interferon alpha:beta signaling, and Guanylate-binding protein, C-terminal.svg)

(<https://string-db.org/cgi/network?taskId=bp4DJurcpbM1&sessionId=brBtz9pZBebx>)

![](case_vs_control/DR/(-)ECM-receptor interaction+Extracellular matrix organization.svg)

(<https://string-db.org/cgi/network?taskId=boMYSHGvJown&sessionId=brBtz9pZBebx>)
...
![](case_vs_control/DR/(-)Interferon alphabeta signaling+Defense response to virus+Antiviral defense.svg)

(<https://string-db.org/cgi/network?taskId=bIbXLj86iCxu&sessionId=brBtz9pZBebx>)
...
Далее сформируем ранки для проведения GSEA.
...
metadata_for_chondro <- read.csv(file = "chondro_metadata/ensbl2geneid.tsv", sep = '\t')
...
В данной таблице представлены результаты GSEA по биологическим процессам и молекулярным функциям (GO).
...
В данной таблице представлены результаты GSEA по клеточным компонентам (GO).
...
Результаты GSEA для базы данных KEGG.
...
Результаты GSEA для базы данных WP.
...
Результаты GSEA по базе данных reactome.
...
![](images/корреляция_коэкспрессия.png)

![](images/профиль_коэкспрессия.png)
...
#| warning: false
#| cache: true
data_green_chondro <- read.csv(file = "case_vs_control/green_module/data_green_chondro.tsv", header = TRUE, sep = '\t')
...
Сохраняем коэкспрессионные модули в таблицу (модуль green).
...
Сохраняем коэкспрессионные модули в таблицу (модуль yellowgreen).
...
#| warning: false
# GSEA
data_genes <- read.csv(file = "chondro_metadata/ensbl2geneid.tsv",
                       header = TRUE, sep = '\t')
net_GM_chondro <- read.csv(file = "case_vs_control/net_GM_chondro.tsv", header = TRUE, sep = '\t')
...
write.table(x = gsea_df_green,file = "case_vs_control/green_module/green_module_coex.xlsx")
...
gsea_greenyellow <- GSEA(geneList = rank_final_GSEA, TERM2GENE = data_greenyellow_chondro[c(2,3)], 
                         eps = 0, pAdjustMethod = "fdr", pvalueCutoff = 0.6)
gsea_df_greenyellow <- data.frame(gsea_greenyellow)
write.table(x = gsea_df_greenyellow,file = "case_vs_control/greenyellow_module_coex.xlsx")
gseaNb(gsea_greenyellow, geneSetID = gsea_greenyellow$ID[1])
```

<p style="text-align: justify;">

Обогащение по модулю yellowgreen и green.
...
#| warning: false
#| cache: true
gsea_df_green <- read.xlsx(xlsxFile = "case_vs_control/green_module/module_green_coex2.xlsx", sheet = TRUE, sep = '\t')
...
#| warning: false
#| cache: true
gsea_df_greenyellow <- read.xlsx(xlsxFile = "case_vs_control/module_yellowgreen_coex2.xlsx", sheet = TRUE, sep = '\t')
...
# data from COex
# green_module
data_coex_green <- read.xlsx(xlsxFile = "case_vs_control/green_module/module_green_coex2.xlsx", colNames = T, sheet = T)
colnames(data_coex_green)[12] <- 'genes'

data_coex_greenyellow <- read.xlsx(xlsxFile = "case_vs_control/module_yellowgreen_coex2.xlsx", colNames = T, sheet = T)
# greenyellow_module
colnames(data_coex_greenyellow)[12] <- 'genes'

# STRING for green_module
data_green_chondro <- read.csv(file = "case_vs_control/green_module/data_green_chondro.tsv", sep = '\t', header = T)
# individual rank for green_module
green_string_data <- read.xlsx(xlsxFile = "case_vs_control/green_module/1. Focal adhesion + Extracellular matrix organization (green_modul_cluster).xlsx", sheet = T, colNames = T)
...
file = "case_vs_control/custom_green_module_df.tsv", quote = F, sep = '\t')
# GSEA
...
![](case_vs_control/green_module/chondro_green_module_general_cluster.svg)

(<https://string-db.org/cgi/network?taskId=bmuQTmdCR7E7&sessionId=brBtz9pZBebx>)
